...
@Component({
  selector: 'app-show-en-compressive-strength',
  standalone: true,
  imports: [
    NgForOf,
    NgIf,
  ...
  templateUrl: './show-en-compressive-strength.component.html',
  styleUrl: './show-en-compressive-strength.component.css'
})
export class ShowEnCompressiveStrengthComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.testService.findById(this.id).subscribe(res => {
      this.test = res;
      if (!this.test.enCompressiveStrengths) {
        this.test.enCompressiveStrengths = [];
      }
    });
